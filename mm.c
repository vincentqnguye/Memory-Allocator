/*-------------------------------------------------------------------
 * Group:
 * 	Vincent Nguyen 30119507
 * 	Arthur Singas  30033264
 *
 * Lab 5 Starter code:
...
void* mm_malloc (size_t size) {
  ...
  ptrFreeBlock = searchFreeList(reqSize); // searches for a free block
  if(ptrFreeBlock == NULL){
	requestMoreSpace(reqSize); // requests for more space if free block is not available
  ...
  blockSize = SIZE(ptrFreeBlock->sizeAndTags); // size of block
  removeFreeBlock(ptrFreeBlock); // removes block from free list

  if((blockSize - reqSize) > MIN_BLOCK_SIZE){ // splits block
	BlockInfo * freeBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
	freeBlock->sizeAndTags = ((blockSize-reqSize) | TAG_PRECEDING_USED) & (~TAG_USED);
	...
	*((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE)))=blockSize-reqSize;
	insertFreeBlock(freeBlock); // inserts the free block
  } else {
	*((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, (blockSize-WORD_SIZE)))=blockSize;
	BlockInfo* nextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize); // pointer to the next block
	nextBlock->sizeAndTags = nextBlock->sizeAndTags | TAG_PRECEDING_USED;
  }

  ptrFreeBlock->sizeAndTags =  ptrFreeBlock->sizeAndTags | TAG_USED; // Sets the block's used tag to true
...
void mm_free (void *ptr) {
  ...
  blockInfo = (BlockInfo*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE); // gets pointer to the header of block
  blockInfo->sizeAndTags=blockInfo->sizeAndTags & (~TAG_USED); // used tag = 0
  insertFreeBlock(blockInfo); // insert block that needs to be freed
  coalesceFreeBlock(blockInfo); // coalesce block with preceding/following free blocks
}
